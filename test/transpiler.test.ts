...
.parent {
  ...
  describe('multiple delimiters', () => {
    it('should handle extra --- in CSS zone (second closes preamble)', () => {
      const { code } = transpile('---\nconst x = 1\n---\nb { color: red; }\n---\nc { color: blue; }');
      expect(code).toContain('b { color: red; }');
      expect(code).toContain('---');  // The extra --- passes through as CSS
      expect(code).toContain('c { color: blue; }');
    });
  });
